...
do
    ...
    fio --name=test --runtime=300 --ioengine=sync --direct=1 --rw=randread --bs=${i} --size=${SIZE} --directory=${DIR} --minimal --output dump
    ...
    echo -n " " >> diskspeed.out
    fio --name=test --runtime=300 --ioengine=sync --direct=1 --rw=randwrite --bs=${i} --size=${SIZE} --directory=${DIR} --minimal --output dump
